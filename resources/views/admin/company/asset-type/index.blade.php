...
<x-company-layout :company="$company">
    <div class="main-fixed-wrap">
        ...
        <div class="main-content">
            <livewire:table :model="App\Models\Admin\Asset\AssetType::class"
            :relations="['workOrderTitles']"
            :where="[
                ['type', '=', config('apg.type.company')],
                ['company_id', '=', $company->id]
            ]" view-file="admin.company.asset-type.table" />
            <livewire:sweet-alert component="table" entityTitle="Asset Type" />
        </div>
    </div>
    <div x-data="addCustomFields()">
        <x-right-modal id="edit-assettype" heading="Edit Asset Type">
    </div>
    </x-right-modal>
    <div x-data="addCustomFields()">
        <x-right-modal id="add-assettype" heading="Add Asset Type">
            <form action="{{ route('admin.companies.asset-types.store', $company->id) }}" method="post" x-data="submitForm()"
                @submit.prevent="onSubmitPost" id="assettype-post-form">
                <div class="form-group">
                    <x-label for="name" value="Name" required="true" />
                    <x-input type="text" class="form-control" x-model="formData.name" id="name" name="name"

<x-app-layout>
    ...
    <div class="main-wrap">
        ...
        <div class="main-wrapper lg:mt-0 mt-[4rem]">
            <div class="admin-tabs">
                <ul class="nav nav-tabs" id="tabs-tab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <a href="?type=information" @class(['nav-link', 'active' => !session()->has('is_upload_media')]) id="tabs-info-tab" data-bs-toggle="pill" data-bs-target="#tabs-info" role="tab" aria-controls="tabs-info" aria-selected="true">INFORMATION</a>
                    </li>
                    <li class="nav-item" role="presentation">
                ...
                <div class="tab-content" id="tabs-tabContent">
                    <div class="tab-pane fade {{ !session()->has('is_upload_media') ? 'show active' : '' }}" id="tabs-info" role="tabpanel" aria-labelledby="tabs-info-tab">
                        <div class="flex md:flex-row flex-col justify-end items-center gap-3">
                            <x-nav-link href="{{ route('admin.assets.edit', $asset->id) }}" class="btn btn-outline-primary text-center">
                                <em class="fa-solid fa-pen"></em> EDIT DETAILS
                            </x-nav-link>
                            <x-nav-link href="{{ route('admin.master-asset.export.pdf', $asset->id) }}" class="btn btn-outline-primary text-center">
                            ...
                            <div class="form-group">
                                <form action="{{ route('admin.assets.upload.media', $asset->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <x-button type="button" class="btn-outline-primary" onclick="document.getElementById('upload-file').click()">
                                        <em class="fa-solid fa-cloud-arrow-up"></em> Upload
                                    </x-button>
                                </form>
                            </div>
                        </div>
                    ...
                    <div class="tab-pane fade" id="tabs-comp" role="tabpanel" aria-labelledby="tabs-comp-tab">
                        <div class="flex justify-end items-center flex-wrap mb-6 gap-3">
                            <a class="md:w-auto w-full" href="{{ route('admin.asset.compliance.pdf', [$asset->id, $asset->asset_type_id]) }}">
                                <x-button type="submit" class="btn-outline-primary">
                                    DOWNLOAD COMPLIANCE PLAN
                                </x-button>
                            </a>
                            <a class="md:w-auto w-full" href="{{ route('admin.asset.grid.compliance.pdf', [$asset->id, $asset->asset_type_id]) }}">
                                <x-button type="submit" class="btn-outline-primary">
                                    {{ Str::upper($asset->name) }} COMPLIANCE PLAN
                                </x-button>
                            </a>
                            <a class="md:w-auto w-full" href="{{ route('admin.asset.detail.compliance.pdf', [$asset->id, $asset->asset_type_id]) }}">
                                <x-button type="submit" class="btn-outline-primary">
                                    DETAIL COMPLIANCE PLAN
                                </x-button>
                            </a>
                            <x-nav-link href="{{ route('admin.asset.work_order.create', $asset->id) }}" class="btn btn-outline-primary text-center">
                                <em class="fa-solid fa-circle-plus"></em> ADD NEW WORK ORDER
                            </x-nav-link>
                        </div>
                        <livewire:work-order-table
                            :where="[['type', '=', config('apg.type.master')]]"
                            view-file="admin.asset.work-order-table"
                            :asset="$asset"
                        />
